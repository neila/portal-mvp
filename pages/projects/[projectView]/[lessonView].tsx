      ...
      <div className="mb-8">
        ...
        <div className="my-4 relative h-8">
          ...
          <Link href={`/projects/${encodeURIComponent(projectView)}/ `}>
            <div className="absolute inset-y-0 right-1/2 transform translate-x-1/2">
...
export async function getStaticProps({ params }) {
  ...
  const lessonContents = await Promise.all(
    lessons.map(async (l, i) => {
      ...
      const isLast =
        i === lessons.length - 1 && sectionCount === parseInt(section.slice(-1))
      return [md, isLast]
    })
  )
  ...
  const nextLessonPath =
    currentLessonIndex === lessons.length - 1
      ? null
      : `${projectView}/${
          lessonView.slice(0, -1) + (currentLessonIndex + 2).toString()
        }`
  const previousLessonPath =
    currentLessonIndex === 0
      ? null
      : `${projectView}/${
          lessonView.slice(0, -1) + currentLessonIndex.toString()
        }`
